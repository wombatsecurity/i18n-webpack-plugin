...
Module.prototype.extractLocales = function ( locales, shared_key ) {
	...
	// pull out "shared" text first
	if ( shared_key && this._json[shared_key] ) {
		shared = this._json[shared_key];
	}

	for ( var locale in this._json ) {
		if ( locale !== shared_key && this._json.hasOwnProperty( locale ) ) {
			// create/get concat_source
			var concat_source;
			if ( !locales[locale] ) {
				concat_source = locales[locale] = new JSONConcatSource();
			} else {
				concat_source = locales[locale];
			}

			// add locale to concat_source
			concat_source.add( new JSONSource( this._json[locale], this._key ) );

			// add shared to locale
			if ( shared ) {
				concat_source.addDefault( this._key, shared );
			}
		}
	}
