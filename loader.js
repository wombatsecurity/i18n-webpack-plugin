...
function loader( source ) {
	...
	// figure out the relative path from the root to the resource
	var relative_path = path.relative( options.root, this.resourcePath ).replace( '/', path.sep );

	// Save the content on the loaderContext (this)
	if ( this[__dirname] ) {
		this[__dirname]( {
			key: relative_path,
	...
	return "module.exports = " + options.fnGenerator( relative_path );
};
 ...
 * @returns {string}
 */
function defaultGetTextGenerator( path ) {
	return "function ( ) { return window.getText('" + path + "'); }";
}
